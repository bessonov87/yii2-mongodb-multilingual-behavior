...
class MultilingualBehavior extends Behavior
{
    ...
    public function createLangClass()
    {
        if (!class_exists($this->langClassName, false)) {
            ...
            // Attributes of database collection
            $mongoDbAttributes = ['_id', $this->languageField, $this->langForeignKey];
            if(!is_array($this->attributes)){
                $this->attributes = explode(',', $this->attributes);
            }
            $mongoDbAttributes = array_merge($mongoDbAttributes, $this->attributes);
            $mongoDbAttributes = array_map(function($val){
                return "'".$val."'";
            }, $mongoDbAttributes);
            $mongoDbAttributes = "[".implode(',', $mongoDbAttributes)."]";
    ...
    public function afterFind()
    {
        ...
        if ($owner->isRelationPopulated('translations') && $related = $owner->getRelatedRecords()['translations']) {
            ...
            // If the entity translated into the language of the visitor, set values of this language for
            // its attributes
            foreach ($this->attributes as $attribute) {
                if($this->{$this->getAttributeName($attribute, $this->currentLanguage)}){
                    $this->setLangAttribute($attribute, $this->{$this->getAttributeName($attribute, $this->currentLanguage)});
                }
            }
